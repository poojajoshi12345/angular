...
$.widget("ibi.ibxSelectMenuButton", $.ibi.ibxMenuButton,
{
	...
	{
		if(this._inMenuSelChange || this._inSetOption)
			return;
	...
	_setOption:function(key, value)
	{
		...
		if(key == "userValue")
		{
			if(!this._inMenuSelChange)
			{
				...
				this._sm.deselectAll();//deselect all (will trigger a menu change with no sel items, and therefore will reset userValue to nothing (temporarily))
				this._sm.selected(selItems, true, true, true);
				options.userValue = userValues;
				this._inSetOption = false;
				return;//the select above will trigger a menu sel change...which will come into _setOption again and correctly set the userValue.
			}
		}
		this._super(key, value);
	},
