...
$.widget("ibi.ibxTree", $.Widget, {
  ...
  _onScroll: function(e) {
    e.preventDefault();
    var scrollerOffset = Math.abs($(this._scroller).offset().top);
  ...
  _renderChunk: function(chunkStartingIndex) {
    ...
    $(df).appendTo(this._virtualListContainer);
  },
  _onFolderClick: function(e) {
    e.stopPropagation();
    var lookupIndex;
    // if the target element is a folder , then we should expand
    if ($(e.target).hasClass("ibfs_folder")) {
      lookupIndex = $(e.target).data("index");
      this._expand(lookupIndex);
    } else if ($(e.target).parent().hasClass("ibfs_folder")) {
      lookupIndex = $(e.target).parent().data("index")
      this._expand(lookupIndex);
    }
  },
  ...
  _buildItem: function(nodeDataIndex, offsetMultiplier) {
    ...
    var newLabel = $("<div class='ibfs-label' style='padding-left:" + (this.options.padding * nodeData.indent) + "px'>")
      .ibxLabel({
        glyph: "",
        glyphClasses: nodeData.getGlyphClasses(),
        wrap: false,
        width: this.options.treeWidth,
        justify: "start",
