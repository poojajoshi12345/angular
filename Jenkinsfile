        ...
        stage("Deploy Artifacts") {
            if("master" == env.BRANCH_NAME) {
                node {
                    ...
                    unstash 'sources'
                    withEnv(["JAVA_HOME=${ tool 'JDK8' }"]) {
                        executeMvnCmd(".", "clean deploy -Dbuild.number=${env.BUILD_NUMBER} -Dbuild.tag=${env.BUILD_TAG} -Dbuild.timestamp=${currentBuild.startTimeInMillis} -DdeployAtEnd ${_mvnOptsSkipTests} -P compress")
                    }                
                }
            }
        }
